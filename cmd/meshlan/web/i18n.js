(() => {
  ...
  const rows = [
    ...
    ['文件', '檔案', 'File', 'ファイル'],
    ['用户 Token 用量', '使用者 Token 用量', 'Token usage by user', 'ユーザー別 Token 使用量'],
    ['Token 用量', 'Token 用量', 'Token usage', 'Token 使用量'],
    ['Token 用量统计', 'Token 用量統計', 'Token usage statistics', 'Token 使用量統計'],
    ['模型输入 Token 是发送给模型的提示词、历史和工具上下文；模型输出 Token 是模型生成的回复，缓存 Token 属于输入的一部分。', '模型輸入 Token 是傳送給模型的提示詞、歷史和工具上下文；模型輸出 Token 是模型產生的回覆，快取 Token 屬於輸入的一部分。', 'Model input tokens are prompts, history, and tool context sent to the model. Model output tokens are the generated reply. Cached tokens are part of the input.', 'モデル入力 Token はモデルへ送るプロンプト、履歴、ツールコンテキストです。モデル出力 Token は生成された応答です。キャッシュ Token は入力の一部です。'],
    ['模型输入（上下文）', '模型輸入（上下文）', 'Model input (context)', 'モデル入力（コンテキスト）'],
    ['模型输出（回复）', '模型輸出（回覆）', 'Model output (reply)', 'モデル出力（応答）'],
    ['模型输入 Token', '模型輸入 Token', 'Model input tokens', 'モデル入力 Token'],
    ['模型输出 Token', '模型輸出 Token', 'Model output tokens', 'モデル出力 Token'],
    ['模型输入', '模型輸入', 'Model input', 'モデル入力'],
    ['模型输出', '模型輸出', 'Model output', 'モデル出力'],
    ['Token 排行榜已刷新', 'Token 排行榜已重新整理', 'Token leaderboard refreshed', 'Token ランキングを更新しました'],
    ['Token 使用排行榜', 'Token 使用排行榜', 'Token usage leaderboard', 'Token 使用量ランキング'],
    ['按已统计总 Token 降序 · 未统计用户不参与排名', '依已統計總 Token 降序 · 未統計使用者不參與排名', 'Ranked by tracked total tokens · untracked users are excluded', '集計済み合計 Token の降順 · 未集計ユーザーは対象外'],
    ['显示全部用户 · 有真实 usage 的按总 Token 排名', '顯示全部使用者 · 有真實 usage 的依總 Token 排名', 'All users shown · users with real usage are ranked by total tokens', '全ユーザーを表示 · 実 usage があるユーザーを合計 Token 順にランキング'],
    ['等待足够的 usage 数据', '等待足夠的 usage 資料', 'Waiting for enough usage data', '十分な usage データを待機中'],
    ['排名', '排名', 'Rank', '順位'],
    ['暂无可排名的 Token 数据', '暫無可排名的 Token 資料', 'No token data available for ranking', 'ランキング対象の Token データなし'],
    ['未统计', '未統計', 'Untracked', '未集計'],
    ['没有真实 usage · 不参与排名', '沒有真實 usage · 不參與排名', 'No real usage · excluded from ranking', '実 usage なし · ランキング対象外'],
    ['次请求', '次請求', 'requests', '件のリクエスト'],
    ['从启用统计功能后开始累计', '從啟用統計功能後開始累計', 'Accumulated after token accounting was enabled', 'Token 集計を有効にしてから累積'],
    ['这里显示模型在响应中报告的真实 usage，不会读取或保存对话正文。', '這裡顯示模型在回應中報告的真實 usage，不會讀取或儲存對話正文。', 'Shows usage reported by the model response. Conversation content is neither read nor stored.', 'モデル応答が報告した usage を表示します。会話本文は読み取りも保存もしません。'],
    ['过去只有流量字节，无法准确换算为 Token，因此不会伪造历史数字。通过 HTTP/HTTPS 域名入口访问且上游返回 usage 的请求可以精确统计；TCP/UDP 透明转发无法识别模型响应，会显示“未统计”。', '過去只有流量位元組，無法準確換算為 Token，因此不會偽造歷史數字。透過 HTTP/HTTPS 網域入口存取且上游返回 usage 的請求可以精確統計；TCP/UDP 透明轉發無法識別模型回應，會顯示「未統計」。', 'Historical traffic contains bytes only and cannot be converted accurately to tokens, so no historical values are fabricated. Requests through the HTTP/HTTPS domain gateway are counted when the upstream returns usage; transparent TCP/UDP forwarding cannot inspect model responses and is marked untracked.', '過去の記録はバイト数のみで Token に正確変換できないため、履歴値は作りません。HTTP/HTTPS ドメイン入口で上流が usage を返したリクエストは正確に集計できます。透過 TCP/UDP 転送ではモデル応答を識別できないため「未集計」と表示します。'],
    ['已统计输入', '已統計輸入', 'Tracked input', '集計済み入力'],
    ['已统计输出', '已統計輸出', 'Tracked output', '集計済み出力'],
    ['已统计总量', '已統計總量', 'Tracked total', '集計済み合計'],
    ['Usage 报告数', 'Usage 報告數', 'Usage reports', 'Usage レポート数'],
    ['输入 Token', '輸入 Token', 'Input tokens', '入力 Token'],
    ['输出 Token', '輸出 Token', 'Output tokens', '出力 Token'],
    ['总 Token', '總 Token', 'Total tokens', '合計 Token'],
    ['缓存 Token', '快取 Token', 'Cached tokens', 'キャッシュ Token'],
    ['推理 Token', '推理 Token', 'Reasoning tokens', '推論 Token'],
    ['已统计请求', '已統計請求', 'Counted requests', '集計済みリクエスト'],
    ['统计状态', '統計狀態', 'Accounting status', '集計状態'],
    ['等待新的 usage 数据', '等待新的 usage 資料', 'Waiting for new usage data', '新しい usage データを待機中'],
    ['精确 · 上游 usage', '精確 · 上游 usage', 'Exact · upstream usage', '正確 · 上流 usage'],
    ['等待上游 usage', '等待上游 usage', 'Waiting for upstream usage', '上流 usage 待ち'],
    ['未统计 · 透明 TCP/UDP', '未統計 · 透明 TCP/UDP', 'Untracked · transparent TCP/UDP', '未集計 · 透過 TCP/UDP'],
    ['暂无用户连接记录', '暫無使用者連線記錄', 'No user connection records', 'ユーザー接続記録なし'],
    ['仅累计已报告 usage 的请求', '僅累計已報告 usage 的請求', 'Only requests reporting usage are accumulated', 'usage を報告したリクエストのみ累積'],
    ['Token 用量读取失败', 'Token 用量讀取失敗', 'Failed to read token usage', 'Token 使用量の読み込みに失敗'],
